...
上一教程对 Android 中的 SQLite 数据库进行了[简介。 本教程以示例说明了所有 CRUD（创建，检索，更新，删除）功能。](https://javatutorial.net/android-sqlite-database-introduction)
...
让我们开始创建用于注册和登录的 xml 布局。如上所述，注册有四个字段和一个确定按钮。 用户单击确定按钮时，将显示一个对话框，因为已保存值，然后将开始登录活动。
