# Raspberry Pi 用 Java 控制直流电动机的速度和方向
...
在本教程中，我将向您展示如何使用 Raspberry Pi 和 Java 控制直流电动机的方向和速度。

我收到了一些要求创建有关使用 Raspberry Pi 和 Java 控制电机速度和方向的教程的请求，所以我们开始吧！ 在本教程中，我将向您展示如何使用 L293D 芯片同时控制和两个电动机的**的方向和速度。**
...
L293D 是用于直流或步进电动机的双 H 桥电动机驱动器，这意味着您可以双向驱动两个直流电动机或一个步进电动机。 L293D 的优点在于，它内置有回弹保护功能，可防止损坏 Raspberry Pi。
...
该芯片有两个+ V 引脚。 引脚号 8（+ Vmotor）为电动机供电，而引脚 16 为芯片本身供电。 我们将引脚 8 连接到电池组，将引脚 16 连接到 Raspberry 的 5V 引脚。

## 选择合适的直流电动机

您需要选择 4.5V 至 36V 的直流电动机。 L293D 无法在 3V 电机上正常工作，并且每个通道的限制为 600mA。
...
Raspberry Pi 3 的硬件设置，以通过 L293D 和 Java 双向控制一台直流电动机

在本教程中，我将使用 GPIO 引脚的物理编号。 物理编号指的是通过简单地从左上角的插针 1 算起并向下计数的插针。

查看下图，该图显示了物理引脚编号和 Pi4J 映射。 例如，物理针脚编号 12 在 Pi4J 中被命名为针脚＃1。 在本教程后面的部分中编写 Java 程序时，请牢记这一点。 您可以在 Pi4J 官方网站中找到有关[引脚编号的更多详细信息。](http://pi4j.com/pins/model-3b-rev1.html)
...
## Java 程序来控制一台直流电动机的方向

这里是有趣的部分。 连接完所有内容后，您可以打开自己喜欢的 Java IDE 并编写代码。 下面的代码将使电动机沿一个方向旋转 3 秒钟，然后使电动机沿相反方向旋转 6 秒钟。
...
如果 pinE 为高电平，则说明电动机已打开。 将 pinE 置于低电平将关闭电动机。 我们通过将引脚 A 或 B 之一切换到高电平并将另一个引脚切换到低电平来确定旋转方向。 例如，如果我们将 pinA 设置为高电平并将 pinB 设置为低电平，则电动机将顺时针旋转。 交换销钉，电动机将以相反的方向旋转。
...
使用 L293D，我们可以独立控制 2 个电机的方向，那么为什么不尝试这样做呢？ 在电路中添加第二个电动机很容易–只需将第二个电动机插入面包板，并使用 3 条额外的电缆将 L293D 芯片的另外 3 个引脚连接到 Raspberry Pi。
...
具有 L293D 和两个直流电动机的 Raspberry Pi 电路

## Java 程序来控制两个直流电动机的方向

我们将我们的第一电机称为`motor1`，第二个电机称为`motor2`。 我们为 Raspberry Pi 分配了 3 个 GPIO 引脚，分别是 0、2 和 3。

下面的 Java 程序将两个电动机沿一个方向旋转 3 秒钟，然后将第一电动机反向旋转 3 秒钟，将第二电动机旋转 6 秒钟。
...
我们在此处使用相同的规则：如果 pinE（启用）为高，则表明电机已打开。 将 pinE 置于低电平将关闭电动机。 我们通过将引脚 A 或 B 之一切换到高电平并将另一个引脚切换到低电平来确定旋转方向。
...
## Java 程序来控制两个直流电动机的速度和方向

最后，我将向您展示如何控制两个直流电动机的**速度和方向**。 我们将为两个电机设置使用相同的电路。 另外，我们将以编程方式依靠 Soft PWM 来控制电动机的速度。 您可以在我的教程中阅读有关 PWM 的更多信息。[具有 PWM 和 Java 的 Raspberry Pi 调光 LED](https://javatutorial.net/raspberry-pi-dim-led-pwm-java)
...
[使用 Pi 和 Python 控制直流电动机](https://business.tutsplus.com/tutorials/controlling-dc-motors-using-python-with-a-raspberry-pi--cms-20051)